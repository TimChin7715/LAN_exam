...
1. `/health` 正常；导入客观题 / 操作题批次、名单（模板见仓库 `templates/`）。
2. 设置页：座位表开关、必要时「清除全部数据」。
3. 创建混合考试（模块 `OBJECTIVE` / `FILL` + 名单 + 起止时间）→ **开始考试**。

**考中**

- 学员登录；座位表（若开启）自动随机分配。
- 客观/操作题自动保存 + 周期 sync；操作题截图佐证（不参与计分）；手动交卷可缺项确认，到点自动交卷规则见 `AGENTS.md`。
- 到点：deadline 自动收卷并 `ENDED`；考官「结束考试」会先强制未交卷学员 deadline 交卷。

**考后**

- 导出成绩 Excel（汇总 + 客观明细 + 操作题明细）；操作题截图 ZIP（仅已交卷）。
...
| 操作题 | Word + 答题卡 Excel + 可选附件 | 工作表名 **答题卡**；列：题号 / 答案 / 分值；答案可用 `\|` 分隔 |
...
| `MAX_PRACTICAL_DOCX_BYTES` | 20MB | 操作题 Word 题目 |
| `MAX_PRACTICAL_XLSX_BYTES` | 10MB | 操作题 Excel / 附件 |
| `MAX_FILLIN_SCREENSHOT_BYTES` | 5MB | 操作题截图单张；每空最多 5 张 |
