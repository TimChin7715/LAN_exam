# Requirements Archive: v1.0 局域网考试 v1.0 MVP
...
- [x] **INFRA-01**: 管理员能在专用服务器上部署并启动系统，考场局域网内任意授权终端通过浏览器访问服务根地址无报错

### 认证与授权

- [x] **AUTH-01**: 教师使用受保护凭据登录管理端；未登录用户无法访问题目与成绩等管理功能
- [x] **AUTH-02**: 学生输入姓名与身份证号后，系统与已导入名单逐字段匹配，仅全部一致时允许进入待参加的考试
...
- [x] **EXAM-01**: 教师能为一次考试关联已导入题目与名单，学生仅能看到属于自己的试卷并完成作答与提交
- [x] **EXAM-02**: 学生提交后不能在未授权情况下修改答卷；同一学生在同一考试重复提交行为有明确提示或拒绝策略

### 导出

- [x] **EXPR-01**: 教师能导出一次考试的成绩汇总（至少包含学生标识、总分、是否提交）
- [x] **EXPR-02**: 教师能导出答题明细（每题作答选项/对错、得分），格式便于存档（如 CSV 或 Excel 之一）
...
| EXPR-01 | Phase 4 | Complete |
| EXPR-02 | Phase 4 | Complete |
...
*Last updated: 2026-05-17 — v1.0 milestone close (all v1 REQ marked shipped)*
