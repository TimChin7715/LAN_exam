...
| express-session + connect-pg-simple | **1.19.0 / 10.0.0** | `sid` + `student_sid` | Phase 3 双 session [VERIFIED: `plugins/session.ts`] |
...
- **用 `teacherId` session 访问学生卷** — 必须 `student_sid` + `rosterEntryId`
...
**How to avoid:** 仅用 `getStudentSession(request)` 写学生字段 [VERIFIED: Phase 3 `session.ts` dual middleware]。
...
| V3 Session Management | yes | HttpOnly `sid` / `student_sid`；regenerate on verify |
