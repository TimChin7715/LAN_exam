  ...
  app.get(
    ...
    async (request, reply) => {
      ...
      const buffer = await readStorageFile(
        exam.fillInBatch.studentExcelStorageKey,
      );
      const downloadName = exam.fillInBatch.excelFileName.replace(
        /\.xlsx$/i,
        '-答题卡.xlsx',
      );
      return reply
