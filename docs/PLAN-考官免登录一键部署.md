...
> 方案版本：2026-05-20（决策修订）  
> **Agent 速览**（已合并提炼）：[AGENTS.md](../AGENTS.md) · Cursor 规则：`.cursor/rules/lan-exam-product.mdc`  
> 根目录 [PLAN-考官免登录一键部署.md](../PLAN-考官免登录一键部署.md) 仅为跳转页。

> 概述：将 `http://127.0.0.1:5180/admin` 作为**仅安装机本机**可操作的免登录管理台；学员经局域网 `http://<管理机IP>:5180/exam` 考试。交付形态为 **离线一体化 Inno Setup（`.exe`）+ 桌面快捷方式 + 托盘常驻**（内含 Node、Postgres、VC++ 运行库，考场**无需外网**）；管理相关 API 仅 loopback。

## 决策记录（已确认）

| # | 议题 | 结论 |
| --- | --- | --- |
| 1 | 交付形态与教官端访问 | **必须**提供 Inno Setup 安装程序（`LAN-Exam-Setup.exe`）与桌面快捷方式；**仅安装机（本机 loopback）** 可正常使用教官端（管理台 UI + 管理/认证 API） |
...
| 学员端 | `http://<本机局域网IP>:5180/exam/*`，**保留**准考证/名单登录 |
| 拓扑 | **单机**：考官机 = 服务器；学员机只访问 LAN IP，不访问管理台 |
| 交付 | **Inno Setup `.exe` 一键安装** + 桌面快捷方式 + 托盘常驻；免 Docker |
| 教官端范围 | **仅安装机本机**（`127.0.0.1`）；考试机只能访问学员路径与学员 API |
...
4. **新增 loopback 中间件 `apps/server/src/plugins/admin-loopback-guard.ts`**
   ...
   - 防止考试机访问 `http://<管理机IP>:5180` 时通过 DevTools 调用教官接口
...
| `AdminRoute` 或布局入口 | 当 `location.hostname` 非 `localhost` / `127.0.0.1` 时展示「教官端请在本机通过桌面快捷方式打开」，不渲染管理功能 |
...
- 教官端不依赖独立端口：管理台仅 `127.0.0.1` 使用；`/api/admin/*` 与 `disabled` 下 `/api/auth/*` 仅 loopback
...
- 防火墙仅开放 5180 给局域网学员流量；教官操作始终通过 `127.0.0.1`
